...
from graph.utils import haversine, apply_component_strategy
from typing import Callable
from .loader import AbstractLoader, LoadOptions, OSMSource
...
class OSMLoader(AbstractLoader):
    """
    OSM Loader to build a graph from a osm file
    """
    def load(self, source: OSMSource, options: LoadOptions) -> Graph:
        """
        ...
        :param source: JSONSource:
            A data source containing:
                ...
                - filter_name: Optional predefined filter to select which ways are included.
                                Valid values:
                ...
                - way_filter: Optional custom filter function.
        ...
        if source.way_filter is None and source.filter_name:
            source.way_filter = get_builtin_way_filter(source.filter_name)
        ...
        for way in root.findall("way"):
            ...
            if source.way_filter and not source.way_filter(tags):
